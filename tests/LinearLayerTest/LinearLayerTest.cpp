...
TEST(LinearLayerTest, test) {
  ...
  const VkDeviceSize weights_buffer_size =
      graph.fc1_in_features * graph.fc1_out_features * sizeof(float);
  ...
  bias_buffer.MapData([&weights, &graph](void* data) {
    memcpy(data, weights.data() + graph.fc1_in_features * graph.fc1_out_features,
           graph.fc1_bias * sizeof(float));
  });
  dst_buffer.MapData(
      [&graph](void* data) { memset(data, 0, graph.fc1_out_features * sizeof(float)); });

  // Create and run compute sum pipeline
  std::unique_ptr<mine::vulkan::Linear> linear = std::make_unique<mine::vulkan::Linear>(
      &context, src_buffer, weights_buffer, bias_buffer, dst_buffer, graph.fc1_in_features,
